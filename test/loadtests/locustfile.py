...
class OrderUser(HttpUser):
    ...
    def create_random_order(self):
        ...
        with self.client.post("/order/", json=customer_data, catch_response=True) as response:
            if not response.ok and customer_data["email"] != "noemailaddress":
    ...
    def create_success_order(self):
        self.client.request_name = "/order/"
        with self.client.post("/order/", json=get_customer_data(cart_id=self.cart_id), catch_response=True) as response:
            if response.ok:
                try:
